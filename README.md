...
- All accounts share the same currency; no currency conversion is performed.
- Account IDs are chosen by the caller (not auto-generated by the server).
- Initial balance must be positive; zero-balance accounts are not permitted.
- Transaction amounts must be positive; a transfer that would leave the source account with a negative balance is rejected.
